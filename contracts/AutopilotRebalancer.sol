...
contract AutopilotRebalancer {
        ...
        returns (uint256 mintedTokenId, uint128 mintedLiquidity, uint256 swapAmountOut)
    {
        ...
        _validateClosePosition(params.closePosition);
    ...
    function _validateClosePosition(CloseParams calldata closePosition) private view {
        (
            ,
            ,
            address token0,
            ...
            ,
            ,
            ) = positionManager.positions(closePosition.tokenId);

        if (token0 != weth || token1 != usdc || fee != POOL_FEE) revert UnsupportedPosition();
        if (tickLower >= tickUpper) revert UnsupportedPosition();
        if (liquidity < closePosition.liquidity) revert InvalidAmount();
    }
