...
- Add a gas-cost guardrail before live execution: estimate current Base gas cost in USD, block or delay normal close+swap+mint when it exceeds a configured limit, ping gas periodically, and design a separate emergency close+swap-only flow for extreme moves where minting a new range is worse than waiting.
